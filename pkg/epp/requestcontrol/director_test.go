...
type mockDirectorDataProducer struct {
	name     string
	produces map[string]any
	consumes map[string]any
}

func (m *mockDirectorDataProducer) TypedName() fwkplugin.TypedName {
	return fwkplugin.TypedName{Name: m.name, Type: "mock"}
}

func (m *mockDirectorDataProducer) Produces() map[string]any {
	return m.produces
}

func (m *mockDirectorDataProducer) Consumes() map[string]any {
	return m.consumes
}

func (m *mockDirectorDataProducer) Produce(ctx context.Context, request *fwksched.InferenceRequest, endpoints []fwksched.Endpoint) error {
	endpoints[0].Put(mockProducedDataKey, mockProducedDataType{value: 42})
	return nil
}

func newMockDirectorDataProducer(name string) *mockDirectorDataProducer {
	return &mockDirectorDataProducer{
		name:     name,
...
func TestDirector_HandleRequest(t *testing.T) {
	...
	tests := []struct {
		...
		dataProducerPlugin       *mockDirectorDataProducer
		...
		},
		{
			...
			targetModelName:   model,
			dataProducerPlugin: newMockDirectorDataProducer("test-plugin"),
		},
		{
